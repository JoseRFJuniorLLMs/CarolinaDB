...
| SPEC-013 authorization (no cryptography needed) | ✅ `PermissionGrant` in the catalog, admin commands checked against the manifest's bootstrap admin, exact tenant/namespace/operation grants enforced at invoke, authorization re-checked at resolve, privileged mutations denied atomically, a tombstoned permission stops authorizing. The client principal travels in the `Admit` entry, so every voter judges the same principal in log order. Gate `QI-AUTHZ` PASS: SEC-02 (tenant/namespace isolation), SEC-09 (current authorization on resolve), SEC-14 (privileged mutation authorization) | `crates/carolina-catalog/src/lib.rs` (`authorization_campaign`), `crates/carolina-node/src/core.rs` |
