...
**Public CI:** [GitHub Actions run 34811627120](https://github.com/JoseRFJuniorLLMs/CarolinaDB/actions/runs/34811627120) for audited main commit `28c23ec` is **green**: Rust 1.89.0 and stable on Linux and Windows, quick qualification, spec lint, bounded TLC model checking, RustSec, and all four libFuzzer smoke targets passed.
...
| Observability (SPEC-001 §63, SPEC-002 §130, SPEC-008 §19): the engine counts invocations by contract meaning (committed, business rejection, admission refusal, identity mismatch, outcome unknown, identity expired), separates the replies served byte-identically from a retained result (`retained_replays`, so real executions are `committed + rejected - retained_replays`; only a leader answers a client's retry, so this counter is per-process and must not be summed across voters) and counts resolve outcomes, evictions and retirements; the node publishes those beside the storage kernel counters (commits, fsyncs, checkpoints, reclaimed versions and journal segments, oldest snapshot), consensus (term, commit/applied index, log base, compaction horizon, followers behind the base) and the catalog (generation, grant active), plus its own queue depths and authorization denials. `NodeStatus` carries them as a `name → value` map over the admin codec, and `carolina node status --addr <host:port>` prints them. No wall-clock latency is recorded and no user value is ever logged | ✅ tests `engine_counts_outcomes_by_contract_meaning`, `node_status_publishes_operational_counters` | `crates/carolina-runtime/src/engine.rs` (`EngineMetrics`), `crates/carolina-node/src/{core,protocol}.rs`, `crates/carolina-cli/src/main.rs` |
...
| QI security (SPEC-013) | ⬜ NOT_RUN — mTLS, credential authentication and cryptographic profiles not implemented; authorization is the separate `QI-AUTHZ` PASS check |
...
| Single-IDC C5 ordered execution (SPEC-008 §8): leader admits an `Invoke` as an ordered `Admit` entry; every voter executes the same deterministic engine step in log order; the leader proposes the unique `Decision` (receipt digest); the client reply waits for that decision's commit; followers verify their own digest and fail closed on divergence; `ResolveRequest` served by the leader only after a read barrier; a follower refuses mutation and a minority cannot acknowledge a final mutation. An indeterminate minority attempt is resolved after quorum recovery before convergence is judged. The process campaign retries its idempotent administrative seed if leadership moves and refreshes the leader before directed checks | ✅ | `crates/carolina-node/src/core.rs`, `crates/carolina-node/src/campaign.rs` |
...
| Tenancy of a running cluster | ⬜ **one hardcoded tenant** — `NodeCore::open` derives the bootstrap tenant `tenant-c5` and takes the first operation's request namespace, and `leader_duties` stages exactly one `PermissionGrant` over them for the principal `dev-local-client`. That is the only identity a client can invoke or resolve under on a live node; any other tenant is refused with `AuthorizationDenied`. There is no operator path to create a tenant, a principal or a further grant, and the connection's principal is not authenticated at all (`DEV_LOCAL` lets an endpoint declare its own role). The catalog itself already supports arbitrary grants (`QI-AUTHZ`); what is missing is the administrative surface and the credential that would make a second principal mean anything | `core.rs` (`open`, `leader_duties`) |
| Real-process campaign (SPEC-010 §9 subset): three `carolina-node` processes, isolated data directories, loopback; C5-001/009/010/021 and replicated determinism after killing the leader, restarting it, killing a majority, and finally taking **every** voter down and bringing the whole cluster back (a cold start is the only case in which a leader is elected over a catalog it has not replayed yet) | ✅ | `crates/carolina-node/src/campaign.rs`, `tests/three_nodes.rs` |
| SPEC-013 authorization (no cryptography needed) | ✅ `PermissionGrant` in the catalog, admin commands checked against the manifest's bootstrap admin, exact tenant/namespace/operation-version grants enforced at invoke, authorization re-checked before resolve and retained-result replay, privileged mutations and mismatched grant identities denied atomically, a tombstoned permission stops authorizing. The client principal travels in the `Admit` entry, so every voter judges the same principal in log order. Gate `QI-AUTHZ` PASS: SEC-02 (tenant/namespace isolation), SEC-09 (current authorization on every disclosure path), SEC-14 (privileged mutation authorization) | `crates/carolina-catalog/src/lib.rs` (`authorization_campaign`), `crates/carolina-node/src/core.rs` |
| SPEC-013 transport security and credentials | ⬜ **not implemented** — no TLS/mTLS (the dependency closure has no TLS crate), no credential records or X.509, no signed payloads, no audit record, no encrypted backup, no rotation and no revocation; the node speaks only the plaintext loopback `DEV_LOCAL` profile with self-declared endpoint roles and never advertises `ENCRYPTED_HOST_V1`. 13 of the 16 SEC rows are untouched, `QI-SECURITY` stays NOT_RUN, so the QI gate does not open and the SPEC-014 §3 exit criteria of MVP-3 are **not met** even though Q3-C5 passes |
...
| QI verdict | Q: `QI-CATALOG` PASS (CAT-01/02/05/12/15/16 + closed-never-reopens); `QI-AUTHZ` PASS (SEC-02/09/14); `QI-CODEC-CORPUS` PASS (54 vectors / 25 kinds; kinds of disabled features are listed as not frozen); `QI-SECURITY` NOT_RUN → gate QI NOT_RUN |
...
- A checkpoint that leaves the buffer pool dirty cannot complete the persisted image, and journal retention only advances on a complete image. That stall used to be silent; `storage.checkpoint_image_incomplete_total` now counts it, and any value above zero is a defect signal rather than a tuning matter — a successful checkpoint writes every page reachable from the root, so a dirty page left over is by definition unreachable.
- A leader runs bootstrap only after it has applied its own log (`applied_index == raft.last_index()`, which holds once the term entry it appended on election is committed and applied). Before that its catalog view is not authoritative: it would see no genesis and propose a second one, which every voter refuses and fails closed on — permanently, since the duplicate entry is durable and every later restart replays it. There is no repair path for a log that already contains such an entry; a cluster in that state has to be rebuilt from outside. Regression: `a_cold_restart_of_every_voter_does_not_bootstrap_twice` plus the cold restart phase of the process campaign.
- A voter that has not yet applied genesis refuses **every** admin request, `Status` included, because `admin_allowed` is answered from the bootstrap manifest the genesis entry carries. A cluster that never bootstraps is therefore opaque to `carolina node status`; only a voter that fails closed *after* genesis can report its own `failure`. Operators have to read the process log in that window.
